...
export const 求年干支 = (年偏移: number, 基准年干支: 干支 = '辛酉'): 干支 => {
  const 基准年序数 = 干支排列表.indexOf(基准年干支)
  const 目标年序数 = (基准年序数 + 年偏移) % 60
  return 干支排列表[目标年序数]
}
...
export const 求月干支 = (月偏移: number, 年干: 天干): 干支 => {
  ...
  const 干支偏移 = (年干偏移 * 12 + 月偏移 + 基准月偏移) % 60
  return 干支排列表[干支偏移]
}
...
export const 求日干支 = (日偏移: number, 基准日干支: 干支 = '甲子'): 干支 => {
  const 基准日序数 = 干支排列表.indexOf(基准日干支)
  const 目标日序数 = (基准日序数 + 日偏移) % 60
  return 干支排列表[目标日序数]
}
...
export const 求时干支 = (时偏移: number, 日干: 天干): 干支 => {
  ...
  const 时干支序数 = (日干偏移 * 12 + 时偏移 + 基础时偏移) % 60
  return 干支排列表[时干支序数]
}
