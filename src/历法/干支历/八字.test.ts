...
test('时间转八字', () => {
  ...
  // 春分前
  const 春分前 = dayjs('2025-02-01 16:00:00')
  const { 八字: 春分前八字 } = 时间转八字(春分前)
  expect(春分前八字).toEqual('甲辰-丁丑-辛丑-丙申')

  // 超过节气范围
  const 超过节气范围 = dayjs('2125-02-01 16:00:00')
  expect(() => 时间转八字(超过节气范围)).toThrow()
})
